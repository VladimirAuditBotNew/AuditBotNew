...
def send_subscribe(message):
    try:
        ...
        # Проверка, если пользователь уже подал заявку в течение суток
        if user_id in pending_requests:
            last_request_time = pending_requests[user_id]["timestamp"]
            current_time = time.time()
            time_passed = current_time - last_request_time
            if time_passed < 24 * 3600:  # Если прошло менее 24 часов
                print(f"Пользователь @{username} пытался повторно отправить /subscribe, но прошло только {time_passed} секунд")
                return  # Игнорируем команду

        # Проверка, находится ли пользователь в процессе подачи заявки
        if user_id in in_progress:
            print(f"Пользователь @{username} уже в процессе подачи заявки")
            return  # Игнорируем команду
...
def handle_subscription_period(call):
    try:
        ...
        if action == "reject":
            ...
            start_date = datetime.now()
            end_date = start_date + timedelta(days=30 * months)
            # Создаём ссылку с истечением срока действия, равным оплаченному периоду
