...
def init_db():
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS resend_requests
                     (user_id INTEGER PRIMARY KEY, username TEXT, timestamp TEXT)''')
        conn.commit()
        conn.close()
...
def check_subscriptions():
    try:
        ...
        # Проверка просроченных запросов на новую ссылку
        current_time = datetime.now().timestamp()
...
def create_resend_link_keyboard(user_id, username):
    ...
    # Убедимся, что user_id — это строка, состоящая только из цифр
    user_id_str = str(user_id)
    if not user_id_str.isdigit():
        print(f"Ошибка: user_id {user_id} не является числом при создании клавиатуры")
        return None
    keyboard.add(InlineKeyboardButton("Подтвердить", callback_data=f"approve_resend_{user_id_str}"))
    keyboard.add(InlineKeyboardButton("Отклонить", callback_data=f"reject_resend_{user_id_str}"))
    print(f"Создана клавиатура для user_id {user_id_str}")
    return keyboard
...
def handle_subscription_period(call):
    try:
        ...
        if result:
            ...
            print(f"Найден пользователь: @{username}, invite_link_used: {invite_link_used}")
        else:
            print(f"Пользователь {user_id} не найден в базе подписок")
            # Проверяем, есть ли пользователь в pending_requests или pending_resend_requests
            if user_id in pending_requests:
                username = pending_requests[user_id]["username"]
            elif user_id in pending_resend_requests:
                username = pending_resend_requests[user_id]["username"]
            if not username:
        ...
        if action == "reject":
            ...
            print(f"Подтверждение запроса на новую ссылку для @{username}")
            if user_id in pending_links:
            ...
            c.execute("DELETE FROM resend_requests WHERE user_id = ?", (user_id,))
            conn.commit()
            print(f"Новая ссылка выдана для @{username} после подтверждения")
        elif action == "reject_resend":
            print(f"Отклонение запроса на новую ссылку для @{username}")
...
def resend_link(message):
    try:
        ...
        print(f"Обработка /resend_link для @{username}, user_id: {user_id}")
        ...
        if not result:
            ...
            conn.close()
            return
        db_username, invite_link_used = result
        print(f"Найден пользователь: @{db_username}, invite_link_used: {invite_link_used}")
        ...
        # Проверяем, есть ли уже запрос на новую ссылку
        if user_id in pending_resend_requests:
        ...
        # Добавляем запрос в оперативную память
        pending_resend_requests[user_id] = {
            "username": username,
            "timestamp": datetime.now().timestamp()
        }
        # Пробуем сохранить запрос в базу данных
        try:
            c.execute("INSERT OR REPLACE INTO resend_requests (user_id, username, timestamp) VALUES (?, ?, ?)",
                      (user_id, username, str(datetime.now().timestamp())))
            conn.commit()
        except Exception as e:
            print(f"Ошибка сохранения запроса в базу данных для @{username}: {e}")

        keyboard = create_resend_link_keyboard(user_id, username)
        if keyboard is None:
            bot.reply_to(message, "Произошла ошибка при создании запроса. Свяжитесь с @AVIT0Master.")
            conn.close()
            return
        ...
        print(f"Ошибка обработки /resend_link: {e}")
        bot.send_message(ADMIN_ID, f"⚠️ Ошибка обработки /resend_link для user_id {user_id}: {e}")
