...
def init_db():
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS subscriptions
                     (user_id BIGINT PRIMARY KEY, username TEXT, start_date TEXT, end_date TEXT, 
                      reminder_sent_1day INTEGER DEFAULT 0, reminder_sent_1hour INTEGER DEFAULT 0)''')
...
def add_subscription(user_id, username, months=1):
    try:
        ...
        c = conn.cursor()
        c.execute(
            "INSERT INTO subscriptions (user_id, username, start_date, end_date, reminder_sent_1day, reminder_sent_1hour) "
            "VALUES (%s, %s, %s, %s, %s, %s) "
            "ON CONFLICT (user_id) DO UPDATE SET username = %s, start_date = %s, end_date = %s, reminder_sent_1day = %s, reminder_sent_1hour = %s",
            (user_id, username, start_date.strftime('%Y-%m-%d %H:%M:%S'), end_date.strftime('%Y-%m-%d %H:%M:%S'), 0, 0,
             username, start_date.strftime('%Y-%m-%d %H:%M:%S'), end_date.strftime('%Y-%m-%d %H:%M:%S'), 0, 0))
        conn.commit()
        print(f"Подписка для @{username} добавлена: start_date={start_date}, end_date={end_date}, months={months}")
        return True
    except Exception as e:
...
def expire_subscription(user_id, username):
    ...
    try:
        ...
        c.execute("UPDATE subscriptions SET end_date = %s WHERE user_id = %s", (today, user_id))
        conn.commit()
...
def check_subscriptions():
    try:
        conn = get_db_connection()
        c = conn.cursor()
        c.execute("SELECT user_id, username, end_date, reminder_sent_1day, reminder_sent_1hour FROM subscriptions")
        subscriptions = c.fetchall()
        now = datetime.now()
        print(f"Проверка подписок на {now}: {len(subscriptions)} записей")
        for user_id, username, end_date, reminder_sent_1day, reminder_sent_1hour in subscriptions:
            ...
            if time_left <= timedelta(days=1) and time_left > timedelta(hours=1) and reminder_sent_1day == 0:
                try:
                    ...
                    c.execute("UPDATE subscriptions SET reminder_sent_1day = 1 WHERE user_id = %s", (user_id,))
                    conn.commit()
                except Exception as e:
                    print(f"Ошибка отправки напоминания за 1 день для @{username}: {e}")

            # Уведомление за 1 час
            if time_left <= timedelta(hours=1) and time_left > timedelta(seconds=0) and reminder_sent_1hour == 0:
                try:
                    bot.send_message(user_id, "Твоя подписка истекает через 1 час! Скорее продли: /subscribe")
                    bot.send_message(ADMIN_ID, f"Подписка @{username} истекает через 1 час!")
                    c.execute("UPDATE subscriptions SET reminder_sent_1hour = 1 WHERE user_id = %s", (user_id,))
                    conn.commit()
                except Exception as e:
                    print(f"Ошибка отправки напоминания за 1 час для @{username}: {e}")
...
def process_payment(message):
    try:
        ...
        conn = get_db_connection()
        c = conn.cursor()
        c.execute("SELECT start_date FROM subscriptions WHERE user_id = %s", (user_id,))
        ...
        if existing_subscription:
            ...
            c.execute("SELECT start_date FROM subscription_history WHERE user_id = %s ORDER BY start_date DESC LIMIT 1", (user_id,))
...
def send_subscribe(message):
    try:
        ...
        print(f"Обработка /subscribe для user_id={user_id}, username=@{username}")
        if not username:
            bot.reply_to(message, "У тебя нет ника (@username). Укажи его в настройках Telegram и попробуй снова.")
            print(f"У пользователя user_id={user_id} нет ника")
            return

        # Проверка, есть ли активная подписка
        conn = get_db_connection()
        c = conn.cursor()
        c.execute("SELECT end_date FROM subscriptions WHERE user_id = %s", (user_id,))
        subscription = c.fetchone()
        print(f"Результат запроса для user_id={user_id}: subscription={subscription}")
        if subscription:
            end_date = subscription[0]
            end_date_dt = datetime.strptime(end_date, '%Y-%m-%d %H:%M:%S')
            now = datetime.now()
            print(f"Сравнение времени: now={now}, end_date_dt={end_date_dt}, now < end_date_dt={now < end_date_dt}")
            if now < end_date_dt:
...
def handle_subscription_period(call):
    try:
        ...
        conn = get_db_connection()
        c = conn.cursor()
        c.execute("SELECT username, end_date FROM subscriptions WHERE user_id = %s", (user_id,))
        result = c.fetchone()
        username = None
        end_date = None
        if result:
        ...
        if action == "reject":
            ...
            start_date = datetime.now()
            end_date = start_date + timedelta(days=30 * months)
            print(f"Выбранный период: {months} месяцев, start_date={start_date}, end_date={end_date}")

            # Проверяем, есть ли уже запись в subscription_history с пересекающимся периодом
            c.execute("SELECT start_date, end_date FROM subscription_history WHERE user_id = %s ORDER BY start_date DESC LIMIT 1", (user_id,))
            ...
            if existing_history:
                ...
                if start_date <= existing_end:
                    ...
                    c.execute("UPDATE subscription_history SET start_date = %s, end_date = %s, months = %s WHERE user_id = %s AND start_date = %s",
                              (start_date.strftime('%Y-%m-%d %H:%M:%S'), end_date.strftime('%Y-%m-%d %H:%M:%S'), months, user_id, existing_history[0]))
                    print(f"Обновлена существующая запись в subscription_history для @{username}: start_date={start_date}, end_date={end_date}, months={months}")
                else:
                    # Если периоды не пересекаются, добавляем новую запись
                    c.execute(
                        "INSERT INTO subscription_history (user_id, username, start_date, end_date, months) VALUES (%s, %s, %s, %s, %s)",
                        (user_id, username, start_date.strftime('%Y-%m-%d %H:%M:%S'), end_date.strftime('%Y-%m-%d %H:%M:%S'), months))
                    print(f"Добавлена новая запись в subscription_history для @{username}: start_date={start_date}, end_date={end_date}, months={months}")
            else:
            ...
            c.execute("UPDATE subscriptions SET start_date = %s, end_date = %s, reminder_sent_1day = 0, reminder_sent_1hour = 0 WHERE user_id = %s",
    ...
    try:
        ...
        for sub in active_subscriptions:
            ...
            if today <= end_date:
                ...
                print(f"Подписка user_id={sub[0]}: start_date={start_date}, end_date={end_date}, months_active={months_active}")
        ...
        for entry in history:
            ...
            if month_key in monthly_earnings:
                ...
                monthly_earnings[month_key] = earnings
            print(f"История подписки: start_date={start_date}, months={months}, earnings={earnings}, month_key={month_key}")
        ...
        bot.reply_to(message, f"Статистика:\n"
                              ...
                              f"\nИстория доходов:\n{history_text}")
        print(f"Статистика отправлена админу: active_count={active_count}, subscription_earnings={subscription_earnings}")
    except Exception as e:
...
def handle_text(message):
    try:
        ...
        user_id = message.from_user.id
        username = message.from_user.username
        print(f"Обработка текста: text='{text}', user_id={user_id}, username=@{username}")
        if text in command_handlers:
            print(f"Вызываем обработчик для команды: {text}")
            command_handlers[text](message)
        else:
            print(f"Команда {text} не найдена в command_handlers")
    except Exception as e:
...
def schedule_subscription_check():
    while True:
        ...
        check_subscriptions()
        print("Проверка завершена, следующая через 60 минут")
        time.sleep(3600)  # Изменено с 600 (10 минут) на 3600 (60 минут)
