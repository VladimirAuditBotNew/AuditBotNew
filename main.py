...
pending_links = {
}  # {user_id: {"link": invite_link, "time_issued": timestamp, "username": username, "attempts": int}}
# Хранилище заявок с таймстемпом
pending_requests = {
}  # {user_id: {"username": username, "timestamp": timestamp}}
...
def init_db():
    try:
        ...
        if 'invite_link_used' not in columns:
            c.execute(
                '''ALTER TABLE subscriptions ADD COLUMN invite_link_used INTEGER DEFAULT 0'''
            )
        if 'added_to_channel' not in columns:
            c.execute(
                '''ALTER TABLE subscriptions ADD COLUMN added_to_channel INTEGER DEFAULT 0'''
            )
        if 'reminder_sent_1day' not in columns:
            c.execute(
                '''ALTER TABLE subscriptions ADD COLUMN reminder_sent_1day INTEGER DEFAULT 0'''
            )
        if 'reminder_sent_10min' not in columns:
            c.execute(
                '''ALTER TABLE subscriptions ADD COLUMN reminder_sent_10min INTEGER DEFAULT 0'''
            )
        conn.commit()
        conn.close()
    ...
    try:
        for user_id, link_info in list(pending_links.items()):
            try:
                ...
                print(
                    f"Ссылка для @{link_info['username']} аннулирована при старте бота"
                )
            except Exception as e:
                print(
                    f"Ошибка аннулирования ссылки для @{link_info['username']}: {e}"
                )
...
def add_subscription(user_id, username, months=1):
    try:
        ...
        c.execute(
            ...
            (user_id, username, start_date.strftime('%Y-%m-%d %H:%M:%S'),
             end_date.strftime('%Y-%m-%d %H:%M:%S'), 0, 0, 0, 0))
        c.execute(
            "INSERT INTO subscription_history (user_id, username, start_date, end_date, months) VALUES (?, ?, ?, ?, ?)",
            (user_id, username, start_date.strftime('%Y-%m-%d %H:%M:%S'),
             end_date.strftime('%Y-%m-%d %H:%M:%S'), months))
        conn.commit()
        conn.close()
        print(
            f"Подписка для @{username} активирована до {end_date} (на {months} месяцев)"
        )
        return True
    except Exception as e:
...
def expire_subscription(user_id, username):
    ...
    try:
        ...
        try:
            bot.send_message(
                user_id, "Твоя подписка завершена. Оплати снова: /subscribe")
        ...
        try:
            bot.send_message(
                ADMIN_ID,
                f"Подписка @{username} истекла. Удали его из канала и группы 'Аудит VIP'."
            )
            print(
                f"Уведомление отправлено админу о завершении подписки @{username}"
            )
        except Exception as e:
            print(
                f"Ошибка отправки уведомления админу о завершении подписки @{username}: {e}"
            )
    except Exception as e:
        print(f"Ошибка завершения подписки для {user_id}: {e}")
        bot.send_message(
            ADMIN_ID,
            f"⚠️ Ошибка завершения подписки @{username}: {e}. Проверь вручную."
        )
    finally:
        if conn:
            conn.close()
...
def check_subscriptions():
    try:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT user_id, username, end_date, reminder_sent_1day, reminder_sent_10min FROM subscriptions"
        )
        ...
        for user_id, username, end_date, reminder_sent_1day, reminder_sent_10min in subscriptions:
            ...
            print(
                f"Проверка пользователя @{username}: конец подписки {end_date}, осталось времени: {time_left}"
            )

            if time_left <= timedelta(days=1) and time_left > timedelta(
                    minutes=10) and reminder_sent_1day == 0:
                try:
                    bot.send_message(
                        user_id,
                        "Твоя подписка истекает через 1 день. Продли: /subscribe"
                    )
                    bot.send_message(
                        ADMIN_ID,
                        f"Подписка @{username} истекает через 1 день.")
                    c.execute(
                        "UPDATE subscriptions SET reminder_sent_1day = 1 WHERE user_id = ?",
                        (user_id, ))
                    conn.commit()
                except Exception as e:
                    print(
                        f"Ошибка отправки напоминания за 1 день для @{username}: {e}"
                    )

            if time_left <= timedelta(minutes=10) and time_left > timedelta(
                    seconds=0) and reminder_sent_10min == 0:
                try:
                    bot.send_message(
                        user_id,
                        "Твоя подписка истекает через 10 минут! Скорее продли: /subscribe"
                    )
                    bot.send_message(
                        ADMIN_ID,
                        f"Подписка @{username} истекает через 10 минут!")
                    c.execute(
                        "UPDATE subscriptions SET reminder_sent_10min = 1 WHERE user_id = ?",
                        (user_id, ))
                    conn.commit()
                except Exception as e:
                    print(
                        f"Ошибка отправки напоминания за 10 минут для @{username}: {e}"
                    )
        ...
        for user_id, link_info in list(pending_links.items()):
            ...
            if time_passed >= 600:  # 10 минут — аннулирование
                try:
                    ...
                    bot.send_message(
                        user_id,
                        "Твоя ссылка истекла. Запроси новую: /resend_link")
                    print(
                        f"Ссылка для @{username} аннулирована (прошло 10 минут с момента выдачи)"
                    )
                    del pending_links[user_id]
                except Exception as e:
        ...
        for user_id, request_info in list(pending_requests.items()):
            ...
            if time_passed >= 24 * 3600:  # Каждые 24 часа
                bot.send_message(
                    ADMIN_ID,
                    f"⏰ Напоминание: обработайте заявку для @{username}. Прошло {int(time_passed // (24 * 3600))} дн."
                )
        ...
        for user_id, request_info in list(pending_resend_requests.items()):
            ...
            if time_passed >= 24 * 3600:  # Каждые 24 часа
                bot.send_message(
                    ADMIN_ID,
                    f"⏰ Напоминание: обработайте запрос на новую ссылку для @{username}. Прошло {int(time_passed // (24 * 3600))} дн."
                )
...
@bot.chat_join_request_handler(
    lambda message: str(message.chat.id) == GROUP_ID)
def handle_join_request(join_request):
    try:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT end_date, added_to_channel, invite_link_used FROM subscriptions WHERE user_id = ?",
            (user_id, ))
        result = c.fetchone()
        if result:
            end_date, added_to_channel, invite_link_used = result
            today = datetime.now()
            end_date_dt = datetime.strptime(end_date, '%Y-%m-%d %H:%M:%S')
            try:
                member_status = bot.get_chat_member(CHANNEL_ID, user_id).status
                is_in_channel = member_status in [
                    'member', 'administrator', 'creator'
                ]
            except Exception as e:
                print(f"Ошибка проверки статуса в канале для @{username}: {e}")
                is_in_channel = False

            if today <= end_date_dt and added_to_channel == 1 and invite_link_used == 1 and is_in_channel:
                bot.approve_chat_join_request(GROUP_ID, user_id)
                print(f"Заявка от @{username} на вступление в группу одобрена")
                bot.send_message(
                    user_id,
                    f"Твоя заявка на вступление в группу одобрена! Добро пожаловать!"
                )
            else:
                bot.decline_chat_join_request(GROUP_ID, user_id)
                if today > end_date_dt:
                    reason = "подписка истекла"
                elif added_to_channel == 0:
                    reason = "не добавлен в канал"
                elif invite_link_used == 0:
                    reason = "ссылка не использована"
                elif not is_in_channel:
                    reason = "ты покинул канал, обнови подписку: /subscribe"
                else:
                    reason = "неизвестная причина"
                print(
                    f"Заявка от @{username} на вступление в группу отклонена ({reason})"
                )
                bot.send_message(
                    user_id,
                    f"Твоя заявка отклонена: {reason}. Обнови подписку: /subscribe"
                )
        else:
            bot.decline_chat_join_request(GROUP_ID, user_id)
            print(
                f"Заявка от @{username} на вступление в группу отклонена (нет подписки)"
            )
            bot.send_message(
                user_id, "У тебя нет активной подписки. Оплати: /subscribe")
        conn.close()
    except Exception as e:
        print(f"Ошибка обработки заявки на вступление: {e}")
        bot.send_message(ADMIN_ID,
                         f"⚠️ Ошибка обработки заявки от {username}: {e}")
        ...
        if str(chat_id) == CHANNEL_ID:
            ...
            if user_id in pending_links:
                ...
                if time_passed < 600:  # Если прошло меньше 10 минут
                    ...
                    print(
                        f"Ожидание {time_to_wait} секунд перед аннулированием ссылки для @{username}"
                    )
                ...
                c = conn.cursor()
                c.execute(
                    "UPDATE subscriptions SET added_to_channel = 1, invite_link_used = 1 WHERE user_id = ?",
                    (user_id, ))
                conn.commit()
                conn.close()
    except Exception as e:
...
def send_welcome(message):
    try:
        if message.chat.type == 'private':
            bot.reply_to(
                message,
                "Привет! Я бот 'Аудит на автопилоте'. Выбери: /subscribe — подписка 500 руб/мес, /premium — личный аудит 2000 руб, /stats — статистика, /help — помощь."
            )
        else:
            bot.reply_to(
                message,
                "Привет! Это канал 'Аудит на автопилоте'. Пиши мне в личку: @AuditAutopilotBot /subscribe."
            )
...
def create_subscription_keyboard(user_id, username):
    ...
    for month in range(1, 13):
        keyboard.add(
            InlineKeyboardButton(f"{month} мес",
                                 callback_data=f"period_{user_id}_{month}"))
    keyboard.add(
...
def create_resend_link_keyboard(user_id, username):
    ...
    keyboard.add(
        InlineKeyboardButton("Подтвердить",
                             callback_data=f"approve_resend_{user_id}"))
    keyboard.add(
        InlineKeyboardButton("Отклонить",
                             callback_data=f"reject_resend_{user_id}"))
    return keyboard
...
def process_payment(message):
    try:
        ...
        if not username.startswith('@'):
            bot.reply_to(
                message,
                "Пожалуйста, укажи свой ник в формате @username (например, @AVIT0Master)."
            )
            return
        username = username[1:]
        if add_subscription(user_id, username, months=1):
            bot.reply_to(
                message,
                "Подписка оформлена! Ожидай подтверждения оплаты от администратора."
            )
            ...
            bot.send_message(
                ADMIN_ID,
                f"@{username} оплатил подписку. Проверь оплату с комментарием 'Audit_@{username}'. Выбери период подписки:",
                reply_markup=keyboard)
        else:
            bot.reply_to(
                message,
                "Не удалось оформить подписку. Напиши @AVIT0Master для помощи."
            )
        print(f"Оплата обработана для @{username}")
    except Exception as e:
        print(f"Ошибка обработки оплаты: {e}")
        bot.send_message(ADMIN_ID,
                         f"⚠️ Ошибка обработки оплаты для @{username}: {e}")
...
def send_subscribe(message):
    try:
        ...
        if not username:
            bot.reply_to(
                message,
                "У тебя нет ника (@username). Укажи его в настройках Telegram и попробуй снова."
            )
            return
        bot.reply_to(
            message,
    ...
    try:
        if call.from_user.id != ADMIN_ID:
            bot.answer_callback_query(
                call.id, "Только администратор может подтверждать оплату.")
            return
        ...
        c = conn.cursor()
        c.execute(
            "SELECT username, invite_link_used FROM subscriptions WHERE user_id = ?",
            (user_id, ))
        result = c.fetchone()
        if not result:
            bot.answer_callback_query(call.id,
                                      "Пользователь не найден в подписках.")
        ...
        if action == "reject":
            bot.send_message(
                user_id,
                "Твоя оплата не подтверждена. Свяжитесь с @AVIT0Master.")
            ...
            if invite_link_used == 1:
                bot.send_message(
                    ADMIN_ID,
                    f"@{username} уже использовал одноразовую ссылку. Если нужно обновить доступ, попроси его оплатить заново: /subscribe"
                )
            ...
            end_date = start_date + timedelta(days=30 * months)
            c.execute(
                "UPDATE subscriptions SET start_date = ?, end_date = ?, reminder_sent_1day = 0, reminder_sent_10min = 0 WHERE user_id = ?",
            ...
            channel_link = bot.create_chat_invite_link(
                CHANNEL_ID, member_limit=1).invite_link
            c.execute(
                "UPDATE subscriptions SET added_to_channel = 1 WHERE user_id = ?",
                (user_id, ))
            conn.commit()
            ...
            bot.send_message(
                ADMIN_ID,
                f"Оплата для @{username} подтверждена на {months} месяцев (до {end_date.strftime('%Y-%m-%d %H:%M:%S')}). Ссылка на канал отправлена пользователю."
            )
            bot.send_message(
                user_id,
                f"Твоя оплата подтверждена! Вступай в канал: {channel_link}\nСсылка активна 10 минут."
            )
            if user_id in pending_requests:
                del pending_requests[user_id]
            ...
            if user_id in pending_links:
                bot.revoke_chat_invite_link(CHANNEL_ID,
                                            pending_links[user_id]["link"])
                print(
                    f"Старая ссылка для @{username} аннулирована перед выдачей новой"
                )
                del pending_links[user_id]
            channel_link = bot.create_chat_invite_link(
                CHANNEL_ID, member_limit=1).invite_link
            pending_links[user_id] = {
                ...
                "username": username,
                "attempts":
                pending_links.get(user_id, {}).get("attempts", 0) + 1
            }
            bot.send_message(
                user_id,
                f"Новая ссылка выдана: {channel_link}\nСсылка активна 10 минут."
            )
            ...
            bot.send_message(
                user_id,
                "Запрос на новую ссылку отклонён. Свяжитесь с @AVIT0Master.")
            bot.send_message(
                ADMIN_ID, f"Запрос на новую ссылку для @{username} отклонён.")
            if user_id in pending_resend_requests:
                del pending_resend_requests[user_id]
        ...
        bot.send_message(
            ADMIN_ID,
            f"⚠️ Ошибка обработки периода подписки для @{username}: {e}")
...
def send_premium(message):
    try:
        ...
        bot.reply_to(
            message,
            "Оплати 2000 рублей на карту: 4276 5201 1247 3919. Пришли чек и текст объявления."
        )
...
def resend_link(message):
    try:
        ...
        if not result:
            bot.reply_to(message,
                         "У тебя нет активной подписки. Оплати: /subscribe")
            return
        username, invite_link_used = result
        if invite_link_used == 1:
            bot.reply_to(
                message,
                "Ты уже использовал одноразовую ссылку. Оплати заново: /subscribe"
            )
            return
        ...
        if user_id in pending_resend_requests:
            bot.reply_to(
                message,
                "Твой запрос на новую ссылку уже отправлен. Ожидай подтверждения от администратора."
            )
            return

        daily_attempts = sum(
            1 for user, info in pending_links.items()
            if info["username"] == username and (
                datetime.now().timestamp() - info["time_issued"]) < 24 * 3600)
        if daily_attempts >= 2:
            bot.reply_to(
                message,
                "Превышен лимит попыток (2 раза в сутки). Обратитесь к @AVIT0Master."
            )
            return
        ...
        bot.reply_to(
            message,
            "Запрос на новую ссылку отправлен администратору. Ожидай подтверждения."
        )
        bot.send_message(
            ADMIN_ID,
            f"@{username} запросил новую ссылку для доступа к каналу. Подтвердить?",
            reply_markup=keyboard)
        ...
        bot.send_message(
            ADMIN_ID, f"⚠️ Ошибка обработки /resend_link для @{username}: {e}")
...
def revoke_all_links_command(message):
    try:
        if message.from_user.id != ADMIN_ID:
            bot.reply_to(
                message, "Только администратор может аннулировать все ссылки.")
            return
        ...
        for user_id, link_info in list(pending_links.items()):
            try:
                bot.revoke_chat_invite_link(CHANNEL_ID, link_info["link"])
                print(
                    f"Ссылка для @{link_info['username']} аннулирована администратором"
                )
            except Exception as e:
                print(
                    f"Ошибка аннулирования ссылки для @{link_info['username']}: {e}"
...
try:
    ...
    init_db()
    revoke_all_links()  # Аннулируем все старые ссылки при старте
    keep_alive()
    print("Бот запущен")
    ...
    while True:
        try:
            ...
            print(f"Ошибка polling, перезапуск через 5 секунд: {e}")
            time.sleep(5)
except Exception as e:
