...
GROUP_LINK = "https://t.me/+mkgmZ69oXJIzZTFi"  # Новая ссылка на группу
...
def handle_subscription_period(call):
    try:
        ...
        bot.send_message(ADMIN_ID, f"⚠️ Ошибка обработки callback для user_id {user_id}: {e}")
...
def resend_link(message):
    try:
        ...
        print(f"Обработка /resend_link для @{username}")
