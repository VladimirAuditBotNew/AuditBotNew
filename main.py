...
# Хранилище ссылок для аннулирования (теперь будет синхронизироваться с базой данных)
pending_links = {}  # {user_id: {"link": invite_link, "time_issued": timestamp, "username": username, "link_used": bool, "expiry_date": expiry_date}}
...
def init_db():
    try:
        ...
        # Новая таблица для pending_links
        c.execute('''CREATE TABLE IF NOT EXISTS pending_links
                     (user_id INTEGER PRIMARY KEY, link TEXT, time_issued TEXT, username TEXT, 
                      link_used INTEGER DEFAULT 0, expiry_date TEXT)''')
        conn.commit()
        conn.close()
...
def load_pending_links():
    try:
        conn = sqlite3.connect('subscriptions.db')
        c = conn.cursor()
        c.execute("SELECT user_id, link, time_issued, username, link_used, expiry_date FROM pending_links")
        links = c.fetchall()
        for user_id, link, time_issued, username, link_used, expiry_date in links:
            pending_links[user_id] = {
                "link": link,
                "time_issued": float(time_issued),
                "username": username,
                "link_used": bool(link_used),
                "expiry_date": expiry_date
            }
        conn.close()
        print(f"Загружено {len(links)} записей в pending_links")
    except Exception as e:
        print(f"Ошибка загрузки pending_links: {e}")

def save_pending_link(user_id, link_info):
    try:
        conn = sqlite3.connect('subscriptions.db')
        c = conn.cursor()
        c.execute(
            "INSERT OR REPLACE INTO pending_links (user_id, link, time_issued, username, link_used, expiry_date) VALUES (?, ?, ?, ?, ?, ?)",
            (user_id, link_info["link"], str(link_info["time_issued"]), link_info["username"], int(link_info["link_used"]), link_info["expiry_date"])
        )
        conn.commit()
        conn.close()
        print(f"Сохранена ссылка для user_id {user_id} в базе данных")
    except Exception as e:
        print(f"Ошибка сохранения pending_link для user_id {user_id}: {e}")

def remove_pending_link(user_id):
    try:
        conn = sqlite3.connect('subscriptions.db')
        c = conn.cursor()
        c.execute("DELETE FROM pending_links WHERE user_id = ?", (user_id,))
        conn.commit()
        conn.close()
        print(f"Удалена запись pending_link для user_id {user_id} из базы данных")
    except Exception as e:
        print(f"Ошибка удаления pending_link для user_id {user_id}: {e}")

def revoke_all_links():
    try:
        conn = sqlite3.connect('subscriptions.db')
        c = conn.cursor()
        c.execute("SELECT user_id, link, username FROM pending_links")
        links = c.fetchall()
        for user_id, link, username in links:
            try:
                bot.revoke_chat_invite_link(CHANNEL_ID, link)
                print(f"Ссылка для @{username} аннулирована при старте бота")
            except Exception as e:
                print(f"Ошибка аннулирования ссылки для @{username}: {e}")
        c.execute("DELETE FROM pending_links")
        conn.commit()
        conn.close()
        pending_links.clear()
...
def handle_new_chat_member(update):
    try:
        ...
        print(f"Получено событие new_chat_members для chat_id: {chat_id}")
        if str(chat_id) == CHANNEL_ID:
            user_id = update.from_user.id
            username = update.from_user.username or str(user_id)
            print(f"Пользователь @{username} (user_id: {user_id}) вступил в канал (chat_id: {chat_id})")
            ...
            if user_id not in pending_links:
                print(f"Нет активной ссылки для @{username} в pending_links")
            ...
            expiry_date = datetime.strptime(link_info["expiry_date"], '%Y-%m-%d %H:%M:%S')
            if now > expiry_date:
                print(f"Ссылка для @{username} истекла (срок действия до {link_info['expiry_date']})")
                bot.send_message(ADMIN_ID, f"Пользователь @{username} вступил по истёкшей ссылке (срок действия до {link_info['expiry_date']}). Удали его из канала и группы 'Аудит VIP'.")
                try:
                    bot.send_message(user_id, "Срок действия твоей ссылки истёк. Оплати подписку заново: /subscribe")
                except Exception as e:
                    print(f"Ошибка отправки сообщения пользователю @{username} об истёкшей ссылке: {e}")
                conn.close()
                return

            print(f"Аннулируем ссылку для @{username}")
            try:
                bot.revoke_chat_invite_link(CHANNEL_ID, invite_link)
                print(f"Ссылка для @{username} аннулирована после вступления")
            except Exception as e:
                print(f"Ошибка аннулирования ссылки для @{username}: {e}")
                bot.send_message(ADMIN_ID, f"⚠️ Ошибка аннулирования ссылки для @{username}: {e}")

            pending_links[user_id]["link_used"] = True  # Отмечаем, что ссылка использована
            save_pending_link(user_id, pending_links[user_id])  # Обновляем в базе
            print(f"Обновляем статус в базе для @{username}")
            ...
            # Отправляем ссылку на группу пользователю
            print(f"Отправляем сообщение пользователю @{username} (user_id: {user_id})")
            try:
                bot.send_message(user_id, f"Ты успешно вступил в канал! Теперь подай заявку в группу 'Аудит VIP': {GROUP_LINK}")
                print(f"Сообщение пользователю @{username} отправлено")
            except Exception as e:
                print(f"Ошибка отправки сообщения пользователю @{username}: {e}")
                bot.send_message(ADMIN_ID, f"⚠️ Ошибка отправки сообщения пользователю @{username}: {e}")

            # Уведомляем админа
            print(f"Отправляем уведомление админу о вступлении @{username}")
            try:
                bot.send_message(ADMIN_ID, f"Пользователь @{username} воспользовался ссылкой ✅ ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')}).")
                print(f"Уведомление админу о вступлении @{username} отправлено")
            except Exception as e:
                print(f"Ошибка отправки уведомления админу о вступлении @{username}: {e}")
                bot.send_message(ADMIN_ID, f"⚠️ Ошибка отправки уведомления админу о вступлении @{username}: {e}")

            conn.close()
    except Exception as e:
        print(f"Ошибка обработки вступления в канал: {e}")
        bot.send_message(ADMIN_ID, f"⚠️ Ошибка обработки вступления в канал для user_id {user_id if 'user_id' in locals() else 'не определён'}: {e}")
...
def process_payment(message):
    try:
        ...
        # Проверяем, не является ли сообщение командой /subscribe
        if text.lower() == "/subscribe":
            bot.reply_to(message, "Пожалуйста, отправь свой никнейм в формате @username (например, @Hoy_Xay), а не команду.")
            # Регистрируем обработчик снова, чтобы пользователь мог отправить никнейм
...
def send_subscribe(message):
    try:
        ...
        # Дополнительная проверка через pending_links
        if user_id in pending_links and pending_links[user_id]["link_used"]:
            conn = sqlite3.connect('subscriptions.db')
            c = conn.cursor()
            ...
            if result:
                ...
                bot.reply_to(message, "Ты уже вступил в канал, но данные о подписке отсутствуют. Свяжитесь с @AVIT0Master.")
            conn.close()
            return

        # Проверка, если пользователь уже подал заявку и ждёт подтверждения
        if user_id in pending_requests:
            last_request_time = pending_requests[user_id]["timestamp"]
            current_time = time.time()
            time_passed = current_time - last_request_time
            if time_passed < 24 * 3600:  # Если прошло менее 24 часов
                # Молча игнорируем повторную команду
                return
...
def handle_subscription_period(call):
    try:
        ...
        if action == "reject":
            ...
            start_date = datetime.now()
            end_date = start_date + timedelta(days=30 * months)
            # Создаём ссылку с истечением срока действия через 7 дней
            link_expiry_date = start_date + timedelta(days=7)
            ...
            pending_links[user_id] = {
                ...
                "username": username,
                "link_used": False,
                "expiry_date": link_expiry_date.strftime('%Y-%m-%d %H:%M:%S')
            }
            save_pending_link(user_id, pending_links[user_id])
            bot.send_message(ADMIN_ID, f"Пользователь @{username} получил ссылку: {channel_link} (действует до {link_expiry_date.strftime('%Y-%m-%d %H:%M:%S')}).")
            bot.send_message(user_id, f"Твоя оплата подтверждена! Вступай в канал: {channel_link} (ссылка действительна до {link_expiry_date.strftime('%Y-%m-%d %H:%M:%S')}).")
            if user_id in pending_requests:
                del pending_requests[user_id]
            if user_id in in_progress:
                del in_progress[user_id]
...
def revoke_all_links_command(message):
    try:
        ...
        for user_id, link, username in links:
            try:
                bot.revoke_chat_invite_link(CHANNEL_ID, link)
                print(f"Ссылка для @{username} аннулирована администратором")
            except Exception as e:
                print(f"Ошибка аннулирования ссылки для @{username}: {e}")
        c.execute("DELETE FROM pending_links")
        conn.commit()
...
try:
    ...
    init_db()
    revoke_all_links()
    load_pending_links()  # Загружаем pending_links из базы данных
