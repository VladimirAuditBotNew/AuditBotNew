...
@bot.chat_join_request_handler(lambda message: str(message.chat.id) == GROUP_ID)
def handle_join_request(join_request):
    try:
        ...
        conn = sqlite3.connect('subscriptions.db')
        c = conn.cursor()
        c.execute("SELECT end_date, added_to_channel, invite_link_used FROM subscriptions WHERE user_id = ?", (user_id,))
        result = c.fetchone()
        if result:
            ...
            try:
                ...
                is_in_channel = member_status in ['member', 'administrator', 'creator']
            except Exception as e:
                ...
                try:
                    ...
                    print(f"Повторная ошибка проверки статуса для @{username}: {e2}")

            # Проверяем, использовал ли пользователь ссылку (из pending_links или базы данных)
            link_used = False
            if user_id in pending_links and pending_links[user_id].get("link_used", False):
                link_used = True
            elif invite_link_used == 1:
                link_used = True

            if today <= end_date_dt and added_to_channel == 1 and link_used and is_in_channel:
                bot.approve_chat_join_request(GROUP_ID, user_id)
                print(f"Заявка от @{username} на вступление в группу одобрена")
                bot.send_message(user_id, f"Твоя заявка на вступление в группу одобрена! Добро пожаловать!")
            else:
                ...
                print(f"Заявка от @{username} на вступление в группу отклонена ({reason})")
                bot.send_message(user_id, f"Твоя заявка отклонена: {reason}. Обнови подписку: /subscribe")
        else:
            bot.decline_chat_join_request(GROUP_ID, user_id)
            print(f"Заявка от @{username} на вступление в группу отклонена (нет подписки)")
            bot.send_message(user_id, "У тебя нет активной подписки. Оплати: /subscribe")
        conn.close()
    except Exception as e:
        print(f"Ошибка обработки заявки на вступление: {e}")
        bot.send_message(ADMIN_ID, f"⚠️ Ошибка обработки заявки от {username}: {e}")
