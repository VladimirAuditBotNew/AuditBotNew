    ...
    try:
        ...
        if existing_subscription:
            ...
            if previous_subscription:
                ...
                bot.send_message(ADMIN_ID, f"Пользователь @{username} уже подавал заявку {start_date} (история).")
    ...
    try:
        ...
        if user_id in pending_requests:
            ...
            if time_passed < 24 * 3600:  # Если прошло менее 24 часов
                ...
                bot.reply_to(message, f"Ты уже подал заявку. Дождись подтверждения администратора или попробуй снова через {hours_left} часов {minutes_left} минут.")
                # Уведомление админу о повторной заявке
