        ...
        if str(chat_id) == CHANNEL_ID:
            ...
            if result:
                ...
                if user_id in pending_links:
                    ...
                    print(f"Нет активной ссылки для @{username}, удаление из канала")
                    try:
...
def handle_subscription_period(call):
    try:
        ...
        if action == "reject":
            ...
            bot.send_message(ADMIN_ID, f"Пользователь @{username} получил ссылку: {channel_link} ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')}).")
